...
resource "openstack_compute_floatingip_associate_v2" "gw_flip" {
   count       = var.config.client_nodes
   floating_ip = "${element(openstack_networking_floatingip_v2.gw_flip.*.address, count.index)}"
   instance_id = "${element(openstack_compute_instance_v2.gw.*.id, count.index)}"
}

resource "openstack_compute_instance_v2" "gw" {
  name            = "consul-gateway-${count.index}"
...
#            "cd /opt/cni/bin ; rm /opt/cni/bin/cni-plugins-linux-adm64-v${local.cni_version}.tgz",
  ...
  provisioner "file" {
     content = templatefile("${path.module}/templates/consul-envoy.service.tpl", {
        datacenter_name = var.config.consul_datacenter_name,
        node_name = "consul-gateway-${count.index}"
        encryption_key = var.config.consul_encryption_key,
        os_domain_name = var.config.os_domain_name,
        floatingip = "${element(openstack_networking_floatingip_v2.gw_flip.*.address, count.index)}",
        privateip = self.access_ip_v4,
        os_region   = "${var.config.os_region}",
     })
     destination = "/etc/systemd/system/consul-envoy.service"
  }
